...
#define EXPECTED_BITS_PER_BLOCK 64
...
template <typename Block, typename Allocator> class dynamic_bitset {
  ...
  BOOST_STATIC_ASSERT(
      (bool)detail::dynamic_bitset_impl::allowed_block_type<Block>::value);
...
public:
  ...
  BOOST_STATIC_CONSTANT(block_width_type,
...
public:
  ...
  dynamic_bitset(
      ...
      size_type num_bits = npos, const Allocator &alloc = Allocator())
  ...
  template <typename BlockInputIterator>
  void m_append(BlockInputIterator first, BlockInputIterator last,
  ...
  {
    if (first != last) {
      typename detail::iterator_traits<BlockInputIterator>::iterator_category
          cat;
  ...
  static void and_parallel(dynamic_bitset &d, const dynamic_bitset &x,
                           const dynamic_bitset &y, int32_t num_cores,
                           int32_t work_per_thread) {
    
    int64_t size = std::min(x.num_blocks(), y.num_blocks());
    SET_THREADS(size, work_per_thread, num_cores);
...
#pragma omp parallel
  ...
  static inline void not_parallel(dynamic_bitset &d, const dynamic_bitset &x,
    ...
    SET_THREADS(x.num_blocks(), work_per_thread, num_cores);
...
#pragma omp parallel
  ...
  size_type count_parallel(int32_t num_cores,
                           int32_t work_per_thread) const BOOST_NOEXCEPT;
  size_type count_parallel(int64_t lower_bound, int64_t upper_bound,
                           int32_t num_cores,
                           int32_t work_per_thread) const BOOST_NOEXCEPT;
  ...
  friend void to_string_helper(const dynamic_bitset<B, A> &b, stringT &s,
                               bool dump_all);
...
private:
  ...
  void init_from_unsigned_long(size_type num_bits, unsigned long value /*,
    ...
    typedef boost::detail::dynamic_bitset_impl::shifter<
        num_type, bits_per_block, ulong_width>
        shifter;
...
template <typename BlockIterator, typename B, typename A>
inline void from_block_range(BlockIterator first, BlockIterator last,
                             dynamic_bitset<B, A> &result) {
...
template <typename Block, typename Allocator>
dynamic_bitset<Block, Allocator>::dynamic_bitset(size_type num_bits,
                                                 unsigned long value,
                                                 const Allocator &alloc)
    : m_bits(alloc), m_num_bits(0) {
...
inline void dynamic_bitset<Block, Allocator>::swap(
    dynamic_bitset<Block, Allocator> &b) // no throw
{
...
template <typename Block, typename Allocator>
void dynamic_bitset<Block, Allocator>::resize(size_type num_bits,
                                              bool value) // strong guarantee
{
...
template <typename Block, typename Allocator>
void dynamic_bitset<Block, Allocator>::append(Block value) // strong guarantee
{
...
template <typename Block, typename Allocator>
dynamic_bitset<Block, Allocator> &dynamic_bitset<Block, Allocator>::
operator&=(const dynamic_bitset &rhs) {
...
dynamic_bitset<B, A> &dynamic_bitset<B, A>::operator>>=(size_type n) {
  ...
  if (n > 0) {
    ...
    std::fill_n(m_bits.begin() + (num_blocks() - div), div,
...
template <typename Block, typename Allocator>
dynamic_bitset<Block, Allocator> &
dynamic_bitset<Block, Allocator>::set_parallel(int32_t num_cores,
                                               int32_t work_per_thread) {
  
  SET_THREADS(num_blocks(), work_per_thread, num_cores); 
  
#pragma omp parallel
  {
    std::fill(m_bits.begin() + THREAD_FIRST(),
              m_bits.begin() + THREAD_LAST(),
              ~Block(0));
  }
...
template <typename Block, typename Allocator>
dynamic_bitset<Block, Allocator> &
dynamic_bitset<Block, Allocator>::reset_parallel(int32_t num_cores,
                                                 int32_t work_per_thread) {
...
template <typename Block, typename Allocator>
inline bool
dynamic_bitset<Block, Allocator>::all_parallel(int32_t num_cores,
                                               int32_t work_per_thread) const {
  if (empty()) {
    return true;
  }
  ...
  int32_t num_threads = SetWorkloadPerThread(
      num_blocks(), work_per_thread, start_position_per_core,
      final_position_per_core, num_cores);
...
#pragma omp parallel
    {
      for (size_type i = start_position_per_core[omp_get_thread_num()],
                     e = final_position_per_core[omp_get_thread_num()];
           i < e; ++i) {
...
template <typename Block, typename Allocator>
inline bool
dynamic_bitset<Block, Allocator>::any_parallel(int32_t num_cores,
                                               int32_t work_per_thread) const {
  SET_THREADS(num_blocks(), work_per_thread, num_cores);
...
#pragma omp parallel
  {
    for (size_type i = THREAD_FIRST(); i < THREAD_LAST(); ++i) {
...
template <typename Block, typename Allocator>
inline bool
dynamic_bitset<Block, Allocator>::none_parallel(int32_t num_cores,
                                                int32_t work_per_thread) const {
...
dynamic_bitset<Block, Allocator>::count() const BOOST_NOEXCEPT {
  ...
  enum {
    no_padding = dynamic_bitset<Block, Allocator>::bits_per_block ==
  ...
  enum {
    mode =
        (no_padding && enough_table_width) ? access_by_bytes : access_by_blocks
  };
...
template <typename Block, typename Allocator>
typename dynamic_bitset<Block, Allocator>::size_type
dynamic_bitset<Block, Allocator>::count_parallel(
    int32_t num_cores, int32_t work_per_thread) const BOOST_NOEXCEPT {
  using detail::dynamic_bitset_impl::table_width;
  using detail::dynamic_bitset_impl::access_by_bytes;
  using detail::dynamic_bitset_impl::access_by_blocks;
  using detail::dynamic_bitset_impl::value_to_type;
  ...
  int64_t start_position_per_core[num_cores],
      final_position_per_core[num_cores], lengths[num_cores],
      results[num_cores], result_count = 0;
  int32_t num_threads = SetWorkloadPerThread(
      num_blocks(), work_per_thread, start_position_per_core,
      final_position_per_core, num_cores);
...
#pragma omp parallel
  {
    results[omp_get_thread_num()] =
        do_count(m_bits.begin() + start_position_per_core[omp_get_thread_num()],
                 lengths[omp_get_thread_num()], Block(0),
                 static_cast<value_to_type<(bool)mode> *>(0));
  }
...
typename dynamic_bitset<Block, Allocator>::size_type
dynamic_bitset<Block, Allocator>::count_parallel(
    int64_t lower_bound, int64_t upper_bound, int32_t num_cores,
    int32_t work_per_thread) const BOOST_NOEXCEPT {
  using detail::dynamic_bitset_impl::table_width;
  using detail::dynamic_bitset_impl::access_by_bytes;
  using detail::dynamic_bitset_impl::access_by_blocks;
  ...
  int64_t start_position_per_core[num_cores],
      ...
      results[num_cores], result_count = 0;
  int32_t num_threads = SetWorkloadPerThread(
      upper_bound - lower_bound, work_per_thread, start_position_per_core,
      final_position_per_core, num_cores);
...
#pragma omp parallel
  {
    results[omp_get_thread_num()] =
        do_count(m_bits.begin() + lower_bound +
                     start_position_per_core[omp_get_thread_num()],
                 lengths[omp_get_thread_num()], Block(0),
                 static_cast<value_to_type<(bool)mode> *>(0));
  }
...
void to_string_helper(const dynamic_bitset<B, A> &b, stringT &s,
  ...
  const size_type len =
      dump_all ? dynamic_bitset<B, A>::bits_per_block * b.num_blocks()
               : b.size();
  s.assign(len, zero);
...
//
template <typename Block, typename Allocator, typename stringT>
inline void to_string(const dynamic_bitset<Block, Allocator> &b, stringT &s) {
...
//
template <typename B, typename A, typename stringT>
inline void dump_to_string(const dynamic_bitset<B, A> &b, stringT &s) {
  to_string_helper(b, s, true /* =dump_all*/);
}

template <typename Block, typename Allocator, typename BlockOutputIterator>
inline void to_block_range(const dynamic_bitset<Block, Allocator> &b,
                           BlockOutputIterator result) {
...
unsigned long dynamic_bitset<Block, Allocator>::to_ulong() const {
  ...
  if (find_next(ulong_width - 1) != npos)
    BOOST_THROW_EXCEPTION(
        std::overflow_error("boost::dynamic_bitset::to_ulong overflow"));
...
dynamic_bitset<Block, Allocator>::max_size() const BOOST_NOEXCEPT {
  ...
  const size_type m =
      detail::dynamic_bitset_impl::vector_max_size_workaround(m_bits);
...
bool dynamic_bitset<Block, Allocator>::is_subset_of(
    const dynamic_bitset<Block, Allocator> &a) const {
  assert(size() == a.size());
  for (size_type i = 0; i < num_blocks(); ++i)
...
dynamic_bitset<Block, Allocator>::m_do_find_from(size_type first_block) const {
  ...
  return i * bits_per_block +
         static_cast<size_type>(boost::lowest_bit(m_bits[i]));
}
...
dynamic_bitset<Block, Allocator>::find_next(size_type pos) const {
  ...
  return fore ? pos + static_cast<size_type>(lowest_bit(fore))
...
std::ostream &operator<<(std::ostream &os,
  ...
  if (os.opfx()) {
    ...
    size_t npad =
        os.width() <= 0 // careful: os.width() is signed (and can be < 0)
...
std::basic_ostream<Ch, Tr> &operator<<(std::basic_ostream<Ch, Tr> &os,
  ...
  if (cerberos) {
    ...
    BOOST_TRY {
      ...
      const bitset_size_type width =
          (os.width() <= 0) ? 0 : static_cast<bitset_size_type>(os.width());
    ...
    BOOST_CATCH(...) { // see std 27.6.1.1/4
      bool rethrow = false;
      BOOST_TRY { os.setstate(ios_base::failbit); }
...
template <typename Block, typename Alloc>
std::istream &operator>>(std::istream &is, dynamic_bitset<Block, Alloc> &b) {
  ...
  if (cerberos) {
    ...
    for (int c = buf->sgetc(); appender.get_count() < limit;
         c = buf->snextc()) {
      ...
      if (c == EOF) {
        ...
        BOOST_TRY { appender.do_append(char(c) == '1'); }
        BOOST_CATCH(...) {
...
bool dynamic_bitset<Block, Allocator>::m_check_invariants() const {
  ...
  if (m_bits.size() > m_bits.capacity() || num_blocks() !=
  calc_num_blocks(size()))
      return false;
  */
  return true;
}

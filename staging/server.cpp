...
constexpr char *STAGING_HOST{"127.0.0.1"};
constexpr char *LOCAL_MEM_DATASET_PATH{"/dev/shm/savime/"};
constexpr char *LOCAL_DISK_DATASET_PATH{"/tmp/staging/"};
...
struct database_saver {
    ...
    void run()
    {
        ...
        try {
            execute(c.conn, (char*)query.c_str());
...
{
    mkdir(LOCAL_MEM_DATASET_PATH, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);  
    mkdir(LOCAL_DISK_DATASET_PATH, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);

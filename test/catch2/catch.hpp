...
public:
  explicit UnaryExpr(LhsT lhs)
    : ITransientExpression{false, lhs
                                  ? true
                                  : false},
      m_lhs(lhs) {}
};
...
struct SectionInfo {
  ...
  std::string
    name;
  std::string
    description; // !Deprecated: this will always be empty
  SourceLineInfo
    lineInfo;
};
...
public:
  ...
  template<
    typename T, typename = typename std::enable_if<std::is_constructible<double,
                                                                         T>::value>::type>
...
struct ContainsMatcher : MatcherBase<std::vector<T>> {
  ...
  ContainsMatcher(std::vector<T> const &comparator) : m_comparator(
    comparator) {}
